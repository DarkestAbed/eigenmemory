...
- `wiki_recall(query)` — search the LLM Wiki before answering questions about past decisions or project context. Keyword full-text search (FTS5), not semantic retrieval — short keyword queries beat prose.
...
1. At the start of a task, recall relevant pages with `wiki_recall`/`wiki_query`.
2. After making a decision or learning a durable fact, remember it with `wiki_remember`/`wiki_ingest` directly — don't rely on a host tool's own automatic memory-saving as the primary path; treat it as a fallback that `wiki_reconcile` absorbs, not a second source of truth to maintain in parallel.
